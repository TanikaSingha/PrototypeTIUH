...
const GameElementsIntro = () => {
  ...
  const { gameElementsIntro } = useSelector((state) => state.tutorial);
    ...
    },
    {
      ...
      content: (
        <>
          <div className="flex flex-wrap items-center justify-center">
            <p className="inconsolata">
              Select any one of the elements to proceed to the next step!
            </p>
            <div>
              <img src={aqua} alt="aqua" className="w-28 h-28" />
            </div>
      ...
      <div className="absolute inset-0 grid grid-cols-3 w-[98%] h-[98%] overflow-hidden m-auto ">
        {ElementMap.map((item, index) => {
          ...
          return (
            <div
              key={index}
              className={`group bg-white/10 flex flex-col items-center justify-center cursor-pointer m-2 rounded-lg transition-transform duration-300 hover:scale-105 ${
                currentStep === index ? " shadow-lg" : ""
              } ${name.toLowerCase()}`}
              onClick={() => {
