      ...
      <div className="absolute inset-0 grid grid-cols-3 w-full h-full">
        {ElementMap.map((item, index) => {
          ...
          return (
            <div
              key={index}
              className={`group bg-white/10 flex flex-col items-center justify-center cursor-pointer m-4 rounded-lg transition-transform duration-300 hover:scale-105 ${currentStep === index ? " shadow-lg" : ""
                } ${name.toLowerCase()}`}
